...
def assign_plugins(request, placeholders, template, lang=None, is_fallback=False):
    ...
    # If no plugin is present in the current placeholder we loop in the fallbacks
    ...
    # If no plugin is present in non fallback placeholders, create defaults if enabled)
    if not plugins:
