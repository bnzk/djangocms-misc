...
if "djangocms_misc.autopublisher" not in settings.INSTALLED_APPS:
    ...
    # if DJANGOCMS_MISC_UNTRANSLATED_MARK_ALL = True, publish all langs
    # (usability thing, mostly)
    @receiver(
        post_publish,
    ...
    def post_publish_handler(**kwargs):
        ...
        if published_language != default_language:
            page_instance.publish(default_language)

    # something has changed, if we are not on the default lang, mark
    # current lang/title as as well
    ...
    def post_ph_operation_handler(
        ...
        if plugin and plugin.language != language:
