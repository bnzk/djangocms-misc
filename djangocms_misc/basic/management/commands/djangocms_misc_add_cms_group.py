...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            const=True,
            default=False,
            help="Superuser CMS permissions (add own users, page permissions, etc).",
        )
        parser.add_argument(
            "--plugin",
            required=False,
            ...
            default=False,
            help="""
Searches for permissions with 'plugin' in their content type's app_label
 (ex.: cmsplugin_googlemaps), adds to group.
""",
        )
        parser.add_argument(
            "--cms",
            ...
            required=False,
            help="""
Searches for permissions with 'cms' in their content type's app_label
 (ex.:djangocms_text_ckeditor), adds to group.
""",
        )
        parser.add_argument(
            "--name",
...
Force overwrite of existing group and it's permissions.""",
    ...
    def handle(self, *args, **options):
        ...
        if groups.count():
            ...
            if groups.count() > 1:
                exit(
                    f"more than one group with name that name "
                    f"({group_name}) found! exiting."
                )
            group = groups.first()
        else:
        ...
        self.stdout.write(logging.INFO, "Group created! ({})".format(group_name))
        self.stdout.write(logging.INFO, "Following permissions were granted:")
        for perm in group.permissions.all():
            self.stdout.write(logging.INFO, "{}".format(perm))
