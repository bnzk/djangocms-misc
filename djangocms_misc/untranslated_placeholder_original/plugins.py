...
def assign_plugins(request, placeholders, template, lang=None, is_fallback=False):
    ...
    _assign_plugins(
        request, untranslated_placeholders, template, lang, is_fallback, True
    )


def _assign_plugins(
    request, placeholders, template, lang=None, is_fallback=False, untranslated=False
):
    ...
    # If no plugin is present in the current placeholder
    # we loop in the fallback languages
    # and get the first available set of plugins
    if not (is_fallback or untranslated) and not (
        hasattr(request, "toolbar") and is_edit_mode(request.toolbar)
    ):
